...
public class JAXBExample {
	...
	private static void unMarshalingExample() throws JAXBException, MalformedURLException, IOException
	{
	    ...
	    for(Row camera : cameraNodes.getRowList())
	    {
	        ...
	        if (camera.getCameraStatus().equalsIgnoreCase("desired") &&
	            camera.getIpCommStatus().equalsIgnoreCase("online")) {
	        	counts[0]++;
	        }
	        if (camera.getCameraStatus().equalsIgnoreCase("desired") &&
		            camera.getIpCommStatus().equalsIgnoreCase("offline")) {
		        	counts[1]++;
		    }
	        if (camera.getCameraStatus().equalsIgnoreCase("desired") &&
		            camera.getIpCommStatus().equalsIgnoreCase("no communication")) {
		        	counts[2]++;
		    }
	    }
